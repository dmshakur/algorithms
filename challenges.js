...
function addTwoNumbers(num1, num2) {
  return typeof(num1 || num2) !== 'number' ? NaN : num1 + num2;
}
...
function reverseUpcaseString(input) {
  var array = input.toUpperCase();
  var endResult = array.split("").reverse();
...
function formatWithPadding(num1, str, num2) {
  ...
  if (num1ToString.length >= num2) {
    return num1ToString;
  }
  var number = num2 - num1ToString.length;
  var add = str;
  for (var i = 0; i < number - 1; i++) {
    str = str.concat(add);
  }
  return str.concat(num1ToString);
}
...
function isPalindrome(palin) {
  let str = palin.toLowerCase().split(" ").join("").split("").reverse().join("");
  if (palin.toLowerCase().split(" ").join("") == str) {
    return true;
  } else {
    return false;
  }
...
function findHighestPriced(arr) {
  ...
  arr.forEach(function(obj, idx) {
    if (arr[idx].price > arr[index].price) {
      index = idx;
    }
  });
  return arr[index];
...
function mapArray(arr, funk) {
  let newArray = [];
  for (var i = 0; i < arr.length; i++) {
    console.log(funk(arr[i]));
    newArray.push(funk(arr[i], i));
  }
  return newArray;
}
...
function isPrime(num) {
  ...
  if (num > 1 && (Math.ceil(num) === Math.floor(num))) {
    return true;
  } else {
    return false;
  }
...
function primeFactors(num) {
  if (num <= 1 || (Math.ceil(num) !== Math.floor(num))) return [];
...
function intersection(arr0, arr1) {
  ...
  arr1.forEach(function(el, idx) {
    if (arr0.includes(el)) arr.push(el);
  });
  return arr;
}
...
function isWinningTicket(arr) {
  for (let el of arr)
    if (!(el[0].includes(String.fromCharCode(el[1])))) return false;
  return true;
}
...
function getNumForIP(str) {
  var ip = str.split('.').reverse();
  for (var i = 1, j = parseInt(ip[0]); i < ip.length; i++) {
    j += parseInt(ip[i]) * (256 ** i);
    if (i === ip.length - 1) return j;
  }
}
...
function toCamelCase(str) {
  let newStr = str.split(/[-_]/g), ret = newStr[0]
  for (let i = 1; i < newStr.length; i++) {
    ret += (newStr[i].charAt(0).toUpperCase() + newStr[i].slice(1))
  }
  return ret
}
...
function countTheBits(num) {
  return num.toString(2).split('').map(n => parseInt(n)).reduce((a, v) => a + v);
}
...
function gridTrip(arr, str) {
  var moves = str.match(/\D\d+/g);
  console.log(moves);
  for (var i = 0; i < moves.length; i++) {
    switch (moves[i][0]) {
      case 'U':
        arr[0] += parseInt(moves[i].slice(1));
        break;
      case 'D':
        arr[0] -= parseInt(moves[i].slice(1));
        break;
      case 'L':
        arr[1] -= parseInt(moves[i].slice(1));
        break;
      case 'R':
        arr[1] += parseInt(moves[i].slice(1));
        break;
...
function addChecker(arr, num) {
  for (var i = 0; i < arr.length; i++) {
    if (arr.includes(num - arr[i])) return true;
  }
  return false;
}
...
Fill up an array whose .length is equal to the task int,
then decrements them all, while getting rid of all that are 0, then adding more from the arrays
Finally, increment var time everytime there is a decrease and time's value after the array is emptied is the correct answer, Easy Peasy Japanesey */
function totalTaskTime(queue, int) {
  var time = 0,
    threads = [];
  while (queue.length !== 0 || (queue.length === 0 && threads.length !== 0)) {
    while ((threads.length < int) && (queue.length !== 0)) {
      threads.push(queue.shift());
    }
    threads = threads.map(val => val - 1).filter((val) => val > 0);
    time++;
  }
  return time;
}
